...
COPY . .
WORKDIR "/src/Services/Restaurant/Restaurant.API"
RUN dotnet build "Restaurant.API.csproj" -c Release -o /app/build

FROM build as unittest
WORKDIR /src/Services/Restaurant/Restaurant.UnitTest

FROM build as functionaltest
WORKDIR /src/Services/Restaurant/Restaurant.FunctionalTests
